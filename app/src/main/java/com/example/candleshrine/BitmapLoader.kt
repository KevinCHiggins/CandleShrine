...
class BitmapLoader {
    ...
    fun save(context: Context, bitmap: Bitmap, filename: String) {
        try {
            ...
            val styleFromDatabase = BitmapFactory.decodeStream(context.openFileInput(context.getString(R.string.temp_style_resized_cropped_filename)))
            Log.d(TAG, "TEST Reopened bitmap width " + styleFromDatabase.width)
        } catch (ioe: IOException) {
            Log.d(TAG, ioe.localizedMessage)
        }
    }
    fun getCentreFitted(context: Context, assetName: String, width: Int, height: Int): Bitmap {
        ...
        // but first of all, workaround for this bug https://stackoverflow.com/questions/39316069/bitmapfactory-decodestream-from-assets-returns-null-on-android-7
        // the workaround is to reopen the InputStream:
